...
export class PainTabStats extends BaseTabStats {
  ...
  report(options: IStatOptions = {}) {
    const stats = this.targetStats(this.analysis.summary.getSpellSummary(SpellId.PAIN), options);

    return this.summaryStats.fields(stats, SpellId.PAIN)
      .concat(this.dotStats.fields(stats, SpellId.PAIN))
      .concat(this.encounterStats.fields(stats, SpellId.PAIN));
  }
}
